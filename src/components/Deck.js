  ...
  return (
    <li className="border rounded-lg shadow">
      ...
      <div className="flex justify-between p-4 bg-gray-100 rounded-b-lg">
        <Link href={`/decks/${deck.id}${user ? '/study' : ''}`} className="inline-flex items-center justify-center rounded-md px-3 py-1.5 font-semibold text-white bg-gray-800 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800">
          {user ? 'Study' : 'Explore'}
        </Link>
        {props.created && (
